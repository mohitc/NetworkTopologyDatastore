...
import io.github.mohitc.topology.impl.primitives.properties.TEPropertyKeyImpl;
...
public class TopologyManagerImpl implements TopologyManager {
  ...
  private static class NEPair {
    ...
    public boolean equals(Object o) {
      if (o != null) {
        if (o instanceof NEPair other) {
          return ((other.aEndID == aEndID) && (other.zEndID == zEndID)) || ((other.aEndID == zEndID) && (other.zEndID == aEndID));
        }
      }
      return false;
    }
  ...
  public Link createLink(int startCpID, int endCpID) throws TopologyException {
    ...
    if (Objects.equals(startNe, endNe)) {
  ...
  public Trail createTrail(int startCpID, int endCpID, PathDTO pathDTO, boolean directed, ConnectionResource resource, NetworkLayer layer) throws TopologyException {
    ...
    synchronized (this) {
      ...
      try {
        ...
        throw new TopologyException("Trail could not be created because of incompatible resource type or unavailable capacity on the path", e);
      }
      //Add IDs for search
  ...
  public void removeAllElements() {
    ...
    neConnections.clear();
    if (log.isInfoEnabled()) {
      log.info("All entities from the topology manager: {} removed successfully", this.getIdentifier());
    }
  }
  ...
  public boolean containsKey(TEPropertyKey key) {
    return Objects.equals(key, registeredKeys.get(key.id()));
  }
  ...
  public void removeKey(String id) throws PropertyException {
    ...
    synchronized (registeredKeys) {
      if (registeredKeys.containsKey(id)) {
        ...
        log.info("Property with id: {} removed", id);
      } else {
