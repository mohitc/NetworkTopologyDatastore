...
public class NotificationHandlerParser {
  ...
  private NotificationProcessorConf parseHandlerNode(Node handler) {
    if (handler.getNodeType() == Node.ELEMENT_NODE) {
      ...
      NotificationHandler handlerObject = createObject(handlerInstance, handlerParams);
  ...
  private NotificationFilter parseFilter(Node filter) {
    ...
    return createObject(filterClass, params);
  }


  private <K> K createObject(Class<? extends K> objectClass, String[] params) {
    try {
      if (params==null) {
  ...
  private <K> Class<? extends K> getValidClass(String className, Class<K> baseType) {
    try {
      ...
      if (baseType.isAssignableFrom(instance)) {
        return instance;
      } else {
        if (log.isErrorEnabled()) {
          log.error("Class {} not a subclass of {}", className, baseType.getSimpleName());
        }
      }
    } catch (ClassNotFoundException e) {
      log.error("Handler class {} not found", className, e);
    }
    return null;
  }
